...
+ (void)GET:(NSString *)requestKey
    ...
    NSString *token = getLocalData(RCToken);
    [mgr.requestSerializer setValue:[NSString stringWithFormat:@"JWT %@",token] forHTTPHeaderField:@"Authorization"];
    [mgr GET:URLStr parameters:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"这个是完整的%@",operation.response.URL);
...
#pragma mark - POST
+ (void)POSTOrder:(NSString *)requestKey
       param:(NSDictionary *)param
     success:(successBlock)success
     failure:(failureBlock)failure
{
    ...
    mgr.requestSerializer.cachePolicy = NSURLRequestReturnCacheDataElseLoad;
    // 因为传递过去和接收回来的数据都不是json类型的，所以在这里要设置为AFHTTPRequestSerializer和AFHTTPResponseSerializer
    mgr.requestSerializer = [AFJSONRequestSerializer serializer];// 请求
    mgr.responseSerializer = [AFJSONResponseSerializer serializer];// 响应
...
#pragma mark - DELETE
+ (void)DELETE:(NSString *)requestKey
         param:(NSDictionary *)param
       success:(successBlock)success
       failure:(failureBlock)failure
{
